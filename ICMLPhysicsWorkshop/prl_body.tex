...
%We compare the pretrained baseline ($4D\_regularized\_5Lremoved$), with the pretrained finetuned version ($4D\_regularized\_5Lremoved_finetuned$).    
...
\begin{figure}[H]
   ...
   \subfigure[$\lambda_{max}$ for ResNet20 layers]{
      \includegraphics[scale=0.13]{img/resnet4d_maxev.png}
      \label{fig:resnet204Dalpha}
   }
   \subfigure[$\alpha$ for ResNet20 layers]{
      \includegraphics[scale=0.13]{img/resnet4d_alphas.png}
      \label{fig:resnet204Dmaxev}l
   }
   \caption{
...
In theoretical physics, many researchers study neural networks using spin glass models, such as the traditional Gardner analysis, [SG's work], etc,
