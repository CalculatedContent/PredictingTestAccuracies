...
\charlesX{TRY THIS FOR LEADIN. We want to predict the trends in the generalization accuracy a series of DNN architectures; how can we do this ?
...
Still perhaps it is not really this hard ?   Recent work by  Liao et al.~\cite{LMBx18_TR} used a data-dependent Product Norm capacity control metric
...
To achieve this result, however, hthey had to modify the DNN optimization loss function.  So this can not be tested on any existing pre-trained DNN architecture,
such as the VGG and ResNet models, widely used today in industry.  Still, these results do suggest that a Product Norm may work well as a capacity
metric for large DNNs with many layers.   Of course, bounding a DNN does not necessarily mean that the individual  weight matrix Norms are 
directly comparable across layers in different architectures.  To predict trends in the test accuracies, one needs some Universal empirical metric.
}

\michael{I WILL REWORK THIS TOO:  to address our main question, we will build upon and combine two seemingly-unrelated lines of recent work.
