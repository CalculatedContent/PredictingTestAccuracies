...
Recall that $\alpha \approx \frac{1}{2}\mu+1$ for VHT random matrices \nred{(Eqn. XXX)}, while $\alpha = a\mu+b$ for MHT random matrices \nred{(Eqn. XXX)} )where $a,b$ strongly depends on $N$ and $M$.
...
\nred{Which is what we want in a practical engineering metric that can describe the average test accuracy.  }
...
\begin{figure*}[!htb]
   \centering
   \subfigure[Random Pareto Matrices] {
      \includegraphics[scale=0.20]{img/fro-rand.png} 
      \label{fig:fro-rand}
   }
   \subfigure[Pre-trained VGG11 Weight Matrices]{
      \includegraphics[scale=0.20]{img/fro-vgg11.png} 
      \label{fig:fro-vgg11}
   }
...
The properties of $\Vert\mathbf{W}\Vert^{2}_{F}$ differ strikingly  for the random Pareto versus real/ron-random DNN weight matrices,
and care must be taken when applying these Universality principles to strongly correlated systems.
...
We present results for eleven more series of pre-trained DNN architectures,
(See Table~\ref{table:models_still_more}), 
eight of which show positive results, as with the VGG and  ResNet
series (above), and three counterexample architectures..

The results that perform as expected are show in
Figures~\ref{fig:models_more}, 
~\ref{fig:still_more_1}, and \ref{fig:still_more_2}.
For each set of models, our Universal metric $\hat{\alpha}$
is smaller when, for the most part, the reported (Top 1)
test accuracy is larger.  This holds approximately
true for the three of the four DenseNet models, with
densenet169 as an outlier.  In fact, this is the only
outlier out of 26 DNN models in these eight architectures.,
...
& densenet201 & 23.1 & \\
\hline
SqueezeNet & squeezenet1\_0 &  & \\
& squeezenet1\_1 &  & \\
 \hline
 CondenseNet & condensenet74\_c4\_g4  &  & \\
 & condensenet74\_c8\_g8 &  & \\
\hline
DPN & dpn68 & 24.17 & \\
& dpn98 & 20.81 & \\
& dpn131 & 20.54 & \\
\hline
ShuffleNet & shufflenetv2\_wd2 &  & \\
& shufflenetv2\_w1 &  & \\
...
\end{tabular}
\end{center}
\caption{Results for more pre-trained DN models.  Models provided in 
the OSMR Sandbox, implemented in pytorch.
        }
...
for the less accurate model. For MeNet, there are seven different models,
and there is no discernible pattern in the data.  Finally, for FDMobileNet,
there are three different pre-trained models,  and, again, the 
 $\hat{\alpha}$ is larger for the less accurate models.
 We have not looked in detail at these results and simply
...
W did not train/retrain lots and lots of (typically rather small) models, analyzing training/test curves, trying to glean from them bits of insight that might then extrapolate to more realistic models.
