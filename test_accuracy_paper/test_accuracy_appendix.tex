...
%%$\alpha \rightarrow 1$, 
$\mu \rightarrow 1$, 
\michael{$\mu$ or $\alpha$, clarify and be consistent with appendix.}
...
\michael{Be careful below about $\mu$s and $\alpha$s throughout this section.}
To start, recall that 
$$ 
\Vert \mathbf{W}\Vert_{F}^{2}=\mbox{Trace}[\mathbf{W}^{T}\mathbf{W}]=N\;\mbox{Trace}[\mathbf{X}]  .
$$
Since, for \nred{$\mu \gtrsim 0$}, 
\michael{NOT $\alpha$, but maybe 1?}
...
$$
\michael{Check that.}
This establishes the approximate---and rather surprising---linear relation we want for $\mu \gtrsim 1$. 
