...
\item
\label{enum:second}
Because the MHT Universality class behaves \emph{like} the VHT Universality class at finite sizes, we show that we can
extend this relation to larger exponents, both across the VHT Universality class, $\mu\in(1,2)$, and into the \emph{finite-sized} MHT Universality class, $\mu\in(2,4)$, 
where it applies for the finite-size weight matrices $\mathbf{W}^{rand}(\mu)$ . For  $N\sim\mathcal{O}(100-1000)$,
$\alpha\;\log\lambda_{max}$  \nred{increases nearly linearly}  with $\log\Vert\mathbf{W}^{rand}(\mu)\Vert^{2}_{F}$  as $\mu$ increases.
For larger $N$, the relation saturates for large $\mu$.
\item
\label{enum:third}
\emph{As evidence of \nred{Heavy Tailed Mechanistic} Universality (HT-MU)}, we observe empirically that this simple relation also applies to the real
 DNN weight matrices $\mathbf{W}$. We see that $\alpha\;\log\lambda_{max}$ 
