...
$$2\log\Vert\mathbf{W}\Vert\approx(-\alpha)\log\lambda_{max}+2\log\lambda_{max}+\log\;K_{\alpha}$$
...
\charlesX{other stuff

\emph{Stable Rank, in Log Units}
$$\dfrac{\log\Vert\mathbf{W}^{rand}\Vert_{F}}{\log\lambda_{max}}$$


$$\dfrac{\log\Vert\mathbf{W}^{rand}\Vert_{F}}{\log\lambda_{max}}\;\;vs.\;\;\alpha$$

The key here is that the slope changes so the plot is flipped
}
...
The numerical results show 3 interesting features. The Power Law-Norm relation
