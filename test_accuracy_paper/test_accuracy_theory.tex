...
\charles{Moved paragraphs around a bit here}\michael{To do.}

\michael{can you please add numbers to these equations:  I use them  in the derivation}\michael{To do.}
...
\michael{Do we use this elsewhere.}
...
\charlesX{Doing so, however, requires care because the pre-trained $\mathbf{W}$ are not heavy tailed random matrices, and the empirical Frobenius norm behaves very differently from a random Pareto matrix.  
Figure~\ref{fig:fnorm} displays the direct relation between the power law exponents and the Frobenius norm (squared) for both randomly generated heavy tailed matrices, and the weight matrices
...
for a random matrix, and is linearly correlated for real DNN data. Which is exactly what we want for our simple complexity metric.
 ...
 \begin{figure}[!htb]
    \centering
    \subfigure[Random Pareto Matrices] {
        \includegraphics[scale=0.3]{img/relation-rand.png} 
        \label{fig:relation-rand}
    }
    \subfigure[VGG 11 Weight matrices]{
        \includegraphics[scale=0.3]{img/relation-vgg11.png} 
...
\paragraph{Proof of the Power Law - Norm Relation}
...
\nred{Below}  we generate a large number of  Heavy-Tailed  random matrices $\mathbf{W}^{rand}(\mu,M)$, 
...
This is an asymptotic relation for the VHT Universality class for very small exponents--but we will extend to larger exponents, both across the VHT and into the MHT Universality class, where it applies for the finite-size weight matrices in DNNs.
}
