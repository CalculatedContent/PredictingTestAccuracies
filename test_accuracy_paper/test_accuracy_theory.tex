...
We demonstrate that this relation holds numerically for both Heavy Tailed random matrices and for 
the weight matrices in pretrained DNNs.  (We also provide a simple proof below, and more details in the Appendix.)

We do this by examining  the Power Law - Norm Relation, in terms of what is effectively
the Soft Rank~\cite{MM18_TR},  expressed in log units as 

$$\alpha\;\sim.\;\dfrac{\log\;\Vert\mathbf{W}\Vert^{2}_{F}}{\log\;\lambda_{max}}$$.

To better understand this relation , and to sketch a proof,
we will generate the data for a number of a Heavy-Tailed random matrices, 
with different power law exponents $\mu$. And we compare
defining correlation matrices $\mathbf{X}$, and therefore $\lambda_{max}$,
