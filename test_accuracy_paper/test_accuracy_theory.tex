...
What does it \emph{mean} to say the DNN weight matrices exhibit Universality? 
This answer to this---and its implications---depends on your perspective---as a Mathematician or a Physicist.
...
It is characterized by experimentally measuring ``observables'' that display HT behavior, with common---or universal---PL exponents. 
...
For this reason, we refer to the Universality observed in HT-SR, i.e., in the ESDs of (pre-trtained) DNNs weight matrices, as \emph{Heavy-Tailed Mechanistic Universality (HT-MU)}.
...
We say that we \emph{model} $\mathbf{W}$ \emph{as if} it is a random matrix, $\mathbf{W}^{rand}(\mu)$, drawn from a Universality class of HT-RMT (i.e., VHT, MHT, or WHT). 
...
Specifically, RMT describes the empirical spectral density, $\rho_{emp}(\lambda)$, its limiting, deterministic form $\rho_{\infty}(\lambda)$, as well as the finite size scaling and fluctuations of the maximum eigenvalue, $\lambda_{max}$~\cite{MM18_TR}. 
...
It also provides simple relations between the empirical observables, e.g., the PL exponent $\alpha$ and the maximum eigenvalue $\lambda_{max}$ of each $\mathbf{W}$, with the parameter(s) $\mu$ of our generative theory, i.e, HT-RMT.   
...
%%HOLD OFF FOR NOW, MAYBE LATER%% \michael{MM ques: maybe derive these 2 relations---Eqns.~(\ref{eqn:alpha_mu_vht_and_mht}) and~(\ref{eqn:scaling_of_lambda_max})---in the appendix.}
...
For $N\sim\mathcal{O}(100-1000)$, $\alpha\;\log\lambda_{max}$ increases nearly linearly with $\log\Vert\mathbf{W}^{rand}(\mu)\Vert^{2}_{F}$ as $\mu$ increases.
For larger $N$, the relation saturates for large $\mu$.  
\item
\label{enum:third}
\emph{As evidence of HT-MU}, we observe empirically that the PL-Norm (Eqn.~(\ref{eqn:basic_relation}) relation also applies, approximately, to the real DNN weight matrices $\mathbf{W}$. 
