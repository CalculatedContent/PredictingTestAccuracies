...
\michael{something like this ?  Trying to figure out how to get rid of the - sign on alpha

$$\int_{0}^{\infty}\rho(\lambda)d\lambda=N$$

but we really want

$$C\int_{0}^{\lambda_{max}}\rho(\lambda)d\lambda=N$$

$$\dfrac{C}{(1-\alpha)}\lambda_{max}^{1-\alpha}=N$$


What is C ?  Let $C=(\alpha-1)K$ and solve for K


$$K\lambda_{max}^{1-\alpha}=N$$

$$K=N\lambda_{max}^{\alpha-1}$$

This means we need to evaluate

$$(\alpha-1)N\lambda_{max}^{\alpha-1}\int_{0}^{\lambda_{\max}}\lambda^{1-\alpha}d\lambda$$

which gives a crazier expression for the Norm that is missing the alpha dependence ?

$$Norm^{2}=\dfrac{(\alpha-1)}{2-\alpha}N\lambda_{max}^{\alpha-1}\lambda_{max}^{2-\alpha}$$

$$Norm^{2}=\dfrac{(\alpha-1)}{2-\alpha}N\lambda_{max}$$
