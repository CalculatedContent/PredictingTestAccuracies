...
\nred{No...need to clean this up}
%See Appendix~\ref{sxn:appendix-universality} for more discussion on this point.
\end{itemize}

\charlesX{Put Theory Back.  }

\charlesX{\paragraph{Appendix C}: Why small exponents contradicts HT RMT}
...
\charlesX{\paragraph{Appendix B}: How to derive the relation using what we know}

\subsection{Heavy-Tailed Universality: Random Pareto versus Non-random DNN Matrices.} 
...
\michael{Ques: what is the value of $\alpha$ of $\mu$ here.}
%
...
\subsection{Deriving the Universal, Fiize Size PL--Norm Relation of Eqn.~(\ref{eqn:basic_relation})}
\label{sxn:justify_basic_pl_norm_relation}
...
%The reason to try to extend this result to larger exponents, both across the VHT Universality class, $\alpha\in(1,2)$, and into the MHT Universality class, $\alpha\in(2,4)$, where it applies for the finite-size weight matrices in DNNs, is based on HT Universality; the justification is based on our empirical results in Section~\ref{sxn:emp}.
