...
Let's review Martin and Mahoney's Theory of Heavy-Tailed Self-Regularization (HT-SR)~\cite{MM18_TR}.
...
See Figure~\ref{fig:power-law-histogram} for a histogram of results for ca. 7500 weight matrices from ImageNet.
...
\begin{figure}[t] %[!htb]
   ...
   \caption{Histogram of fitted PL exponents $\alpha$ for ca. 7500 Linear and 2D Convolutional Layers from ImageNet.  The vast majority have $\alpha\in(1.5,5)$.  Some Conv2D layers have smaller values of $\alpha$; and larger values of $\alpha$ (up to ca. 20) correspond to less reliable fits. \michael{Zoom in for fig, so don't go above 10, and mention small versus large $\alpha$.} }
...
In particular, strongly-correlated DNN weight matrices are not random matrices, but the latter can be used to \emph{model} the former (since, if they are members of the same Universality class, then scaling relations should hold for both).
See Figure~\ref{fig:universality_diagram} for an illustration.
Although we use HT-RMT, we use Universality in the way used in Statistical Physics, i.e., to derive Universal relations between empirically-measurable~quantities.
...
To characterize this behavior, we use a HT variant of the RMT, where we use HT random matrices to elucidate different Universality classes.
...
The HT matrix models were first introduced in the Statistical Physics literature, where they are called \'L\'evy Matrices when $0<\mu<2$~\cite{PB94}.
...
%%
\michael{Cite a bunch of HT-RMT papers here.}
...
At all finite sizes, however, $\alpha$ is still linear in $\mu$, but it displays \emph{very} strong finite-size effects, giving 
\begin{align}
\alpha=a\mu+b, 
\label{eqn:alpha_mu_mht}
\end{align}
...
Finally, for both the VHT and the MHT Universality classes, the maximum empirical eigenvalue, $\lambda_{max}$, follows a Frechet distribution (i.e., an exponentially-truncated PL); and we expect it to scale with $N$ according to Extreme Value Theory (EVT)~\cite{heavytails2007,Resnick07,MM18_TR}:
\begin{equation}
\lambda_{max}\sim N^{4/\mu-1}  
\label{eqn:scaling_of_lambda_max}
\end{equation}
(where, for simplicity, $Q=1$).
\michael{Do we make $Q$ dependence explicit here and elsewhere.}
...
\michael{Maybe derive these 2 relations---Eqns.~(\ref{eqn:alpha_mu_vht_and_mht}) and and~(\ref{eqn:scaling_of_lambda_max})---in the appendix.}

Let's clarify Eqns.~(\ref{eqn:alpha_mu_vht_and_mht}) and and~(\ref{eqn:scaling_of_lambda_max}):  
...
Our goal will be to use this to derive a relationship that holds between $\alpha$, $\lambda_{max}$, and perhaps other observables such as matrix norms, and then appeal to Universality to apply it to non-random pre-trained DNN weight matrices that also lie within these Universality classes.
...
\footnote{There are three reasons we don't want a simple unweighted average: an unweighted average behaves differently for random matrices than well-trained DNN weight matrices, and so it would not be Universal; we want a metric that relates the $\alpha$ of HT-SR Theory with known capacity control metrics such as norms of weight matrices; and unweighted averages, while sometimes providing predictive quality, do not perform reliably well. }
...
\end{equation}
To our knowledge, this is the first time this PL--Norm relation has been noted in the literature.
Two comments on Eqn.~(\ref{eqn:basic_relation}).
...
Second, it shows that the fitted PL exponent can alternatively be interpreted as (up to the $\frac{1}{N}$ scaling) approximately the Stable Rank in Log-Units:
$$
...
\label{enum:third}
Third, 
\emph{based on Universality}, we expect this result to extend to larger exponents, both across the VHT Universality class, $\alpha\in(1,2)$, and into the finite-size MHT Universality class, $\alpha\in(2,4)$, where it applies for the finite-size weight matrices in DNNs.
\end{enumerate}
We will discuss each of these in more detail below.

Based on Eqn.~(\ref{eqn:basic_relation}), we choose the weights in Eqn.~(\ref{eqn:alpha_hat_generic}) to be the log of the corresponding maximum eigenvalues of $\mathbf{X}$, and we define the complexity metrics for Linear and Convolutional Layers as follows.
For Linear~Layers, we have
$$
...
For Conv2D Layers, since we relate the ``norm'' of the 4-index Tensor $\mathbf{W}_{l}$ to the sum of the $n_{L}=c\times d$ terms for each feature map, we have
$$
...
\begin{equation}
2\log\mathcal{C}=\langle\log\Vert\mathbf{W}\Vert^{2}_{F}\rangle\rightarrow\hat{\alpha} := \dfrac{1}{N_{\alpha}}\sum_{i,l}\log\lambda^{max}_{i,j}\alpha_{i,l}  .
...
%% 
Given these connections, we can use $\hat{\alpha}$ to analyze numerous pre-trained DNNs.
...
\michael{Be careful below about $\mu$s and $\alpha$s.}
%
...
\michael{Again, be careful below about $\mu$s and $\alpha$s.}




\paragraph{The PL-Norm Relation: Random Matrix versus Real Data.}
...
\begin{figure}[t] %[!htb]
    ...
    \subfigure[VGG 11 Weight matrices]{
        ...
        \caption{Relation between $\alpha\;\log_{10}\;\lambda_{max}$ and the Frobenius Norm squared random (Pareto) matrix and real (VGG11) DNN weight matrices.
                 \michael{Maybe adjust X-axis on \ref{fig:relation-rand}.}
                 }
    \label{fig:relations}
\end{figure}

Here, we show numerically that the general form of Eqn.~(\ref{eqn:basic_relation}) holds more generally than the special case derived previous, including into the MHT Universality class, for both random and real data.
To illustrate this, we generate a large number of HT random matrices $\mathbf{W}^{rand}(\mu)$, with varying sizes $M$ (with, for simplicity, aspect ratio $Q=1$), drawn from a Pareto distribution of Eqn.~(\ref{eqn:ht_dstbn}), with exponents $\mu\in[0.5, 5]$.
\michael{Why are we using $M$ here.} 
...
We also performed a similar PL fit for VGG 11 weight matrices (See Section~\ref{sxn:emp} for some details).
Figure~\ref{fig:relation-vgg11} shows the results, demonstrating an increasing relation until $ \alpha\log\lambda_{max}$ and a saturation after that point.
Overall, the results in Figure~\ref{fig:relations} illustrate that multiplying $\alpha$ by $\log_{10}\lambda_{max}$ leads to a relation that increases nearly linearly with the (log of the squared) Frobenius norm for a random matrix, and that the two quantities are linearly correlated for real DNN weight matrices.
...
\paragraph{Finite-Size Effects Within and Across HT Universality Classes.}
...
\begin{figure}[t] %[!htb]
   ...
   \caption{
            ...
            \michael{Maybe adjust X axis to stop at 5.  Also, we should probably use $N$ rather than $M$ for consistency with the text.}
           }
   \label{fig:randW}
\end{figure}

Here, we consider finite-size effects in Eqn.~(\ref{eqn:basic_relation}), for both VHT and MHT matrices.
See Figure~\ref{fig:randW}, which  displays $\frac{\log\Vert\mathbf{W}\Vert^{2}_{F}}{\log\lambda_{max}}$ as a function of the fitted PL exponent $\alpha$.
...
%% $$
The numerical results for the the PL-Norm Relation show that as $\alpha$ increases:
...
Observe that for MHT matrices, the regime in which it holds moderately well is when $N,M$ is on the order of a hundred to a thousand, which is a typical value for modern DNNs. 
