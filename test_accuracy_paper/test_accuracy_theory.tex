...
For an illustration \nred{of what we mean} by HT-MU, See Figure~\ref{fig:universality_diagram} 
\ngreen{
...
\ngreen{HT-RMT provides more than HT Universality classes; it also provides simple relations between the empirical observables, i.e. 
 the power law exponent $\alpha$ and the maximum eigenvalue $\lambda_{max}$ of each $\mathbf{W}$, with the parameter(s) $\mu$
...
To resolve this apparent discrepancy, we will exploit HT \nred{Mechanistic} Universality to propose a Universal DNN complexity metric.
...
Third, it shows that the fitted PL exponent can alternatively be interpreted (up to the $\frac{1}{N}$ scaling) \nred{as} the Stable Rank in Log-Units:
$$
...
For  $N\sim\mathcal{O}(100-1000)$, $\alpha\;\log\lambda_{max}$  \nred{increases nearly linearly}  with $\log\Vert\mathbf{W}^{rand}(\mu)\Vert^{2}_{F}$  as $\mu$ increases.
For larger $N$, the relation saturates for large $\mu$.  
\item
\label{enum:third}
\emph{As evidence of \nred{Heavy Tailed Mechanistic} Universality (HT-MU)}, we observe empirically that the PL-Norm (Eqn.~(\ref{eqn:basic_relation})  relation also 
applies, approximately, to the real DNN weight matrices $\mathbf{W}$. We see that $\alpha\;\log\lambda_{max}$ 
   \nred{is positively} correlated with $\log\Vert\mathbf{W}\Vert^{2}_{F}$  as $\alpha$  increases, and even shows similar saturation effects at large $\alpha$.
\end{enumerate}
...
for an \nred{$N \times M$} random matrix $\mathbf{W}$, \nred{with $M=N, Q=1$, and} with elements drawn from Eqn.~(\ref{eqn:ht_dstbn}).%
...
$$.
Since, for \nred{$\mu \gtrsim 0$}, \michael{NOT $\alpha$, but maybe 1?} the eigenvalue spectrum is dominated by a single large eigenvalue, it follows that
$$
...
In particular, for MHT matrices, the finite-size regime in which it holds moderately \nred{well when $N,M\sim\mathcal{O}(100-1000)$, which is typical} for modern DNNs. 
