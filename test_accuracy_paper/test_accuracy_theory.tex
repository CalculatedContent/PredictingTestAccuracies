...
$$\langle\log\Vert\mathbf{W}\Vert\rangle=\dfrac{1}{L}\sum_{i=1}^{L}\log_{10}\Vert\mathbf{W}_{i}\Vert$$
...
But this integral is difficult to evaluate.  To get an expression we can work with, we will use a trick from statistical mechanics.

\paragraph{The Annealed Approximation} lets us interchange the log and the integrand when computing the expected value 

$$ \log\mathbb{E}[x]\leftrightarrow\mathbb{E}[\log(x)] $$

So when evaluating our power law integral, we have

$$\log\int\lambda^{1-\alpha}d\lambda\approx\int\log\lambda^{1-\alpha}d\lambda$$
$$=\int\log\dfrac{1}{\lambda^{\alpha-1}}d\lambda$$
$$=-\int\log\lambda^{\alpha-1}d\lambda$$
$$\approx-\log\int\lambda^{\alpha-1}d\lambda$$

This lets us now write

\charles{Somehow I lost a minus sign. Something is off...because the norm can not be negative, and $\alpha>1$}

$$\log\Vert\mathbf{W}\Vert^{2}\approx\log\left[\dfrac{\lambda^{\alpha}}{\alpha}\right]^{\lambda_{max}}_{\lambda_{min}}$$

Since $\lambda_{min}\sim 0$, we have \charles{check this}

$$\log\Vert\mathbf{W}\Vert^{2}=2\log\Vert\mathbf{W}\Vert\approx\alpha\log\lambda_{max}$$
...
$$2\log_{10}\Vert\mathbf{W}\Vert\approx(1-2)\times\alpha$$
...
But first, some numerical examples of the relation between $\langle\log\Vert\mathbf{W}\Vert\rangle$ and $\alpha$
...
this is in the PowerLawExample notebook in the repo
