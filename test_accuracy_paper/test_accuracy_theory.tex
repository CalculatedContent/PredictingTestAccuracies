...
\charles{Or we could just use the square of the Frobenius norm as the VC complexity metric :

$$\mathcal{C}\sim\Vert\mathbf{W}^{2}_{1}\Vert\times\Vert\mathbf{W}^{2}_{2}\Vert\cdots\Vert\mathbf{W}^{2}_{L}\Vert$$

which gives

$$\langle\log\Vert\mathbf{W}\Vert^{2}\rangle\rightarrow\hat{\alpha}$$

where

$$\hat{\alpha}:=\dfrac{1}{N_{\alpha}}\sum_{i,l}b_{i,j}\alpha_{i,l}$$

which would make the presentation easier, but may have some theoretical complications.  .I have not decided yet the best way top write this up}

We can now use $\hat{\alpha}$ ,  or, equivalently, $\langle\log\Vert\mathbf{W}\Vert\rangle$ to analyze numerous pre-trained DNNs...and the results are indeed surprising.
