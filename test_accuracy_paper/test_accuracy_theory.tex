...
Frobenius norm behaves very differently from a random Pareto matrix.  Figure XXX displays the direct relation between
 the power law exponents and the Frobenius norm for both randomly generated heavy tailed matrices, and the weight matrices
 from the pre-trained VGG16 DNN.   
 (NEED TO ADD FIGURE)
 For a random Pareto matrix $\mathbf{W}^{rand}(\mu)$,  the Frobenius norm  $\Vert\mathbf{W}\Vert_{F}$ increases with 
...
We now apply our relation betweemn $\alpha$ and $\mu$ for the Very Heavy Tailed Universality class
...
We can now form a Taylor Series for $\dfrac{1}{4/\mu-1}$ around $\mu=1.15\approx 1$, which gives 

$$\dfrac{1}{4/\mu-1}\bigg\rvert_{\mu=1.15}\approx\dfrac{1}{2}\mu-\dfrac{1}{6}+\cdots\approx\dfrac{1}{2}\mu$$

And we have now proved the linear relation we want for $\mu >= 1$ 
}
